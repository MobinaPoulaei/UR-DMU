...
class WSAD(Module):
    ...
    def forward(self, x):
        if len(x.size()) == 4:
            b, n, t, d = x.size() # n=10, t=32, d=2048
            x = x.reshape(b * n, t, d)
        else:
            b, t, d = x.size() # t=32, d=2048
            n = 1
        x = self.embedding(x)
        x = self.selfatt(x)
        if self.flag == "Train":
            ...
            # print(f'============= self.cls_head(x).shape: {self.cls_head(x).shape}=============')
            # print(f'============= self.cls_head(x).reshape((b, n, -1)).shape: {self.cls_head(x).reshape((b, n, -1)).shape}=============')
            pre_att = self.cls_head(x).reshape((b, n, -1)).mean(1)
            # print(f'============= pre_att: {pre_att.shape}=============') 1, 32
